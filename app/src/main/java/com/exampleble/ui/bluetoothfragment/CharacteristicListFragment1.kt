...
class CharacteristicListFragment1: Fragment() {
            ...
            AdapterView.OnItemClickListener { parent, view, position, id ->
                ...
                if (propList.size > 1) {
                    AlertDialog.Builder(requireActivity())
                        .setTitle(requireActivity().getString(R.string.select_operation_type))
                        .setItems(propNameList.toTypedArray(),
                            DialogInterface.OnClickListener { dialog, which ->
                                (getActivity() as MobileActivity).setCharacteristic(
                                    characteristic
                                )
                                (getActivity() as MobileActivity).setCharaProp(propList[which])
                                (getActivity() as MobileActivity).changePage(3)
                            })
                        .show()
                } else if (propList.size > 0) {
                    (getActivity() as MobileActivity).setCharacteristic(characteristic)
                    (getActivity() as MobileActivity).setCharaProp(propList[0])
                    (activity as MobileActivity).changePage(3)
                }
            }
    }

    fun showData() {
        val service = (activity as MobileActivity).getBluetoothGattService()
    ...
    private inner class ResultAdapter internal constructor(private val context: Context) :
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
            ...
            holder.txt_title!!.setText((requireActivity().getString(R.string.characteristic) + "（" + position + ")").toString())
            ...
            if (property.length > 0) {
                holder.txt_type!!.setText((requireActivity().getString(R.string.characteristic) + "( " + property.toString() + ")").toString())
